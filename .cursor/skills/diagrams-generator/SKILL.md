...
## 扩展能力：多种 Python 绘图库支持

见 [references/python-libraries.md](references/python-libraries.md)，涵盖 matplotlib / seaborn / plotly / graphviz / networkx 的动态选择策略、示例代码和输出格式指南。
...
见 [references/tikz-templates.md](references/tikz-templates.md)，涵盖：
- 神经网络基础模板
- Transformer Encoder-Decoder 完整模板（V5 最佳实践）
- Python 自动化编译脚本
- matplotlib 快速替代方案（无需 LaTeX）

## TikZ 学术图表美化指南

详见 [references/styling-guide.md](references/styling-guide.md)。

---

## Related Skills

- **[mydrawio](../mydrawio/SKILL.md)**: 需要编辑 `.drawio` 格式图表时使用（diagrams-generator 生成静态图，mydrawio 适合可交互/可编辑的 draw.io 图）
- **[pptx](../pptx/SKILL.md)**: 生成的图表需要插入 PPT 演示文稿时使用
