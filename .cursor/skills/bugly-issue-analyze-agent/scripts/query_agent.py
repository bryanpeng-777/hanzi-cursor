...
class BuglyAgentClient:
    ...
    def __init__(self, product_id, base_url=None):
        ...
        self.headers = {
            ...
            "X-Skill-Name": "bugly-issue-analyze-agent",
        }
    ...
    def _load_token(cls) -> str:
        """
        Token 加载优先级：
          1. 环境变量 BUGLY_USER_TOKEN（Secrets 密钥注入，若有则同时持久化到本地缓存文件）
          2. 环境变量 BOX_BUGLY_USER_TOKEN（Box Token 方式）
          3. 本地缓存文件 ~/.bugly_token_cache.json
        """
        # 优先级1: BUGLY_USER_TOKEN（Secrets 方式）
        token = os.environ.get("BUGLY_USER_TOKEN")
        if token:
            cls._save_token_cache(token)
            return token

        # 优先级2: BOX_BUGLY_USER_TOKEN（Box Token 方式）
        token = os.environ.get("BOX_BUGLY_USER_TOKEN")
        if token:
            print(f"[提示] 使用 Box Token 方式加载 BUGLY_USER_TOKEN")
            cls._save_token_cache(token)
            return token
        ...
        if token:
            print(f"[提示] 未检测到环境变量 BUGLY_USER_TOKEN 或 BOX_BUGLY_USER_TOKEN，已从缓存文件 {cls._TOKEN_CACHE_FILE} 加载 token。")
            return token

        raise EnvironmentError(
            "未找到 BUGLY_USER_TOKEN。\n"
            "请通过以下方式之一配置 Token：\n"
            "  1. 推荐：在 Token 管理中配置 BUGLY_USER_TOKEN，Skill 加载时自动注入\n"
            "  2. Box Token 方式：BOX_BUGLY_USER_TOKEN 环境变量会自动注入\n"
            "  3. 手动设置环境变量：export BUGLY_USER_TOKEN=<your_token>\n"
    ...
    def query(self, message, agent_id="94", thread_id=None, run_id=None, verbose=False):
        """
        向 Bugly Agent 发送查询消息并获取响应

        Args:
            message: 用户消息文本
            agent_id: Agent ID（默认 "94"）
        ...
        payload = {
            "thread_id": thread_id,
            "run_id": run_id,
            "messages": [{"id": uuid.uuid4().hex, "role": "user", "content": message}],
            "tools": [],
            "context": [],
            "forwarded_props": {"agent_id": agent_id, "scene": "4"},
            "state": None,
        }
    ...
    def query_text(self, message, agent_id="42", thread_id=None, run_id=None):
        """
    ...
    if result.get("tool_results"):
        ...
        for tr in result["tool_results"]:
            ...
            if len(content_str) > 300:
                content_str = content_str[:300] + f"... (省略，共 {len(content_str)} 字符)"
