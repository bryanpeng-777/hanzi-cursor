...
description: 将 ~/.claude/skills 中的技能同步到 WorkBuddy（.workbuddy/skills/）、CodeBuddy（.codebuddy/skills/）与 Cursor 项目仓库（.cursor/skills/ 与 .cursor/agents/，供 Cloud Agent 使用）；并在每次同步时保证 ~/.claude-internal 下的 skills、agents、knowledge 为指向 ~/.claude 对应目录的符号软链（单一数据源，不 rsync 复制）。当用户提到「更新技能」「更新技能库」「同步技能」「sync skills」「同步 agents」「把技能/agents 同步到 WorkBuddy/CodeBuddy/Cursor」「同步到其他工具」「skills 同步」「skill 同步」「agents 同步」时触发。若用户表述为「更新」或明确要求拉取最新，必须先在本机 ~/.claude 仓库执行 git pull，再 rsync；仅说「同步」且未要求更新远端时，可直接 rsync。WorkBuddy/CodeBuddy/Cursor 侧 skills 按技能目录增量镜像；Cursor 侧 agents 按 .md/.mdc 文件镜像；internal 侧仅维护软链。
---

# Sync Skills to Tools

将 `~/.claude/skills/` 中的技能同步到 WorkBuddy、CodeBuddy 与 Cursor 项目仓库；将 `~/.claude/agents/` 中的 workflow / subagent 定义同步到 Cursor 项目的 `.cursor/agents/`（供 Cloud Agent 读取）。并在**同一次脚本执行**中，保证 `~/.claude-internal/skills`、`~/.claude-internal/agents`、`~/.claude-internal/knowledge` 三者为指向 `~/.claude/skills`、`~/.claude/agents`、`~/.claude/knowledge` 的**符号软链**（绝对路径），**不再**对 internal 做目录复制或 rsync。
...
| Cursor 项目 agents 目标 | `<项目根>/.cursor/agents/`（rsync 镜像 `*.md` / `*.mdc`，含子目录） |
...
| 同步模式 | WB/CB/Cursor skills：按顶层技能目录（含分组如 `camp/`）增量 rsync；Cursor agents：按 `*.md` / `*.mdc` 文件镜像（含子目录，如 `moat-analysis-workflow/step1-level1.md`）；internal：仅 `ln -s`（若原为实体目录则改名为 `*.pre-symlink.时间戳` 后建链） |
...
`--pull-first` 会在 rsync 之前进入 `~/.claude`（若存在 `.git`），否则进入 `~/.claude/skills`，执行 `git pull --ff-only`；若均不是 git 仓库则打印警告并继续同步。
...
# 仅同步到 Cursor 项目 .cursor/skills/ 与 .cursor/agents/（不拉 git；仍会确保 internal 三软链）
...
由 `sync_agents_to` 实现：将 `~/.claude/agents/` 下所有 `*.md` / `*.mdc`（含子目录）镜像到各 Cursor 项目的 `.cursor/agents/`。源目录不存在时跳过并提示。

Cursor 项目路径由 `CURSOR_PROJECT_ROOTS` 控制（默认 `hanzi-cursor`）。同步完成后若用户要求提交，进入对应项目根目录执行：

```bash
git add .cursor/skills .cursor/agents && git commit -m "chore: 同步 skills 与 agents 供 Cloud Agent 使用"
```
...
2. **同步统计**：WorkBuddy/CodeBuddy/Cursor 侧 skills 的新增、更新、跳过；Cursor 侧 agents 的新增、更新、删除；internal 三软链状态（新建 / 已正确 / 跳过源不存在 / 是否发生备份改名）
...
Cursor skills (<项目>/.cursor/skills/)：
  ...

Cursor agents (<项目>/.cursor/agents/)：
  ...
...
- **Cursor Cloud Agent**：项目级 `.cursor/skills/` 与 `.cursor/agents/` 须提交到 Git 后 Cloud Agent 才能读取；用户级 `~/.cursor/skills/`、`~/.claude/agents/` 在云 VM 不可用。Cloud Agent 读 workflow / subagent 时用 `.cursor/agents/<name>.md`，本地 IDE 仍用 `~/.claude/agents/<name>.md`。
