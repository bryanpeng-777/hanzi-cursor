...
**推荐策略**:
- **单看板、数据量大（> 5000 行）**：优先使用下载导出模式，获取原始 CSV 不受行数限制。
- **多看板批量拉取（如日常巡检）**：优先使用页面抓取模式，**串行**逐个执行。`beacon_download_export.py` 在多进程并发或短时间串行场景下容易卡死（"未匹配到任务，等待刷新"循环），而 `beacon_page_scrape.py` 串行运行更稳定可靠。
...
- **Python 3.8+**：使用 `python3` 命令（不是 `python3.8`，后者在部分环境中不存在）
...
```bash
python3.8 {SKILL_DIR}/scripts/beacon_qr_server.py --start \
...
```bash
# 直接下载 (页面默认时间范围)
python3.8 {SKILL_DIR}/scripts/beacon_download_export.py \
  --auth "{SKILL_DIR}/runtime/beacon_auth_state.json" \
  --url "<灯塔页面URL>" \
  --output-dir "<输出目录>" \
  --trigger-export
...
```bash
# 直接抓取页面默认数据
python3.8 {SKILL_DIR}/scripts/beacon_page_scrape.py \
  --auth "{SKILL_DIR}/runtime/beacon_auth_state.json" \
  --url "<灯塔页面URL>" \
  --output-dir "<输出目录>" \
  --wait 15
...
```bash
python3.8 {SKILL_DIR}/scripts/beacon_login.py \
