...
使用方法:
  python3.8 beacon_qr_server.py --start \
...
# ── playwright 自举：缺失时自动切换到 skill venv ──
def _bootstrap_playwright():
    try:
        ...
        print("ERROR: playwright 未安装。请运行:", file=sys.stderr)
        print(f"  uv venv {Path(__file__).resolve().parent.parent}/.venv", file=sys.stderr)
        print(f"  uv pip install playwright --python {Path(__file__).resolve().parent.parent}/.venv/bin/python", file=sys.stderr)
        sys.exit(1)
  ...
  try{
    ...
    switch(d.status){
      ...
      case 'need_scan':
        ...
        if(d.message && d.message.includes('iOA')){
          st.innerHTML='📲 '+d.message;
          icon.textContent='📲';
          title.textContent='iOA 推送确认';
          sub.textContent='已自动发送验证请求，请在手机上打开 iOA App 并点击确认';
        }else{
...
def _do_qr_scan_flow(page, context, browser, auth: Path,
    ...
    # 检测是否是 iOA 推送认证页面（无二维码，需点击按钮）
    try:
        body_text = page.inner_text("body")
        is_ioa_push = any(kw in body_text for kw in [
            "Send verification request", "iOA Mobile", "confirm with iOA"
        ])
    except Exception:
        is_ioa_push = False

    if is_ioa_push:
        log("[扫码流程] 检测到 iOA 推送认证页，自动点击发送验证请求...")
        try:
            # 尝试点击 "Send verification request" 按钮
            btn = page.locator("button", has_text="Send verification request").first
            if btn.count() > 0:
                btn.click()
                log("[扫码流程] ✅ 已点击发送验证请求，请在手机 iOA App 上确认")
            else:
                # fallback: 找所有按钮
                ...
                for b in btns:
                    try:
                        txt = b.inner_text()
                        if "verif" in txt.lower() or "send" in txt.lower() or "confirm" in txt.lower():
                            b.click()
                            log(f"[扫码流程] ✅ 已点击按钮: {txt}")
                            break
                    except Exception:
                        continue
        except Exception as e:
            log(f"[扫码流程] ⚠️ 点击按钮失败: {e}")
        time.sleep(2)
    ...
    # 更新状态为 need_scan
    if is_ioa_push:
        StatusManager.set("need_scan", "iOA推送已发送，请在手机 iOA App 上点击确认")
    else:
...
def _cleanup_old_resources(port: int, cdp_port: int, runtime_dir: Path):
    ...
    if status_file.exists():
        try:
            ...
            pass
    # 也清理旧的 /tmp 下的信号文件（向后兼容）
    old_signal = Path("/tmp/beacon_auth_status")
