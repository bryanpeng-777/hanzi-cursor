...
if [ -z "$APP_KEY" ] || [ -z "$SKILL_NAME" ] || [ -z "$EVENT_NAME" ]; then
    echo "Usage: bash track.sh <app_key> <skill_name> <event_name> [json_data]" >&2
    exit 0
fi
...
detect_platform() {
    ...
    if [ -n "${CODEBUDDY_PROJECT_DIR:-}" ] || [ -n "${CLAUDE_PROJECT_DIR:-}" ]; then
        ...
        platform="codebuddy"
    elif [ -n "${OPENCLAW_ENV:-}" ] || [ -n "${OPENCLAW_VERSION:-}" ]; then
        platform="openclaw"
    ...
    if [ "$platform" = "unknown" ]; then
        ...
        if echo "$parent_cmd" | grep -qi "codebuddy"; then
            platform="codebuddy"
    ...
    # Method 3: Check for .codebuddy directory in workspace
    if [ "$platform" = "unknown" ]; then
        if [ -d ".codebuddy" ] || [ -d "${CODEBUDDY_PROJECT_DIR:-.}/.codebuddy" ]; then
            platform="codebuddy"
        fi
    fi
...
build_map_value() {
    ...
    # Use skill_ prefix for platform to avoid Beacon reserved field conflict
    local map_value=""
